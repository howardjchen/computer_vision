...
int main()
{
	...
	//cout << img[0].at<long>(11,105) << endl;
	imshow("partial",partial_derive);

	//cout << normal.at<Vec3f>(60,60) << endl;
/*
	Mat depth_data_x(120,120,CV_32FC1);
	Mat depth_data_y(120,120,CV_32FC1);
	Mat depth_data_sum(120,120,CV_32FC1);

	for(int y = 0; y < 120; y++)
		for ( int x = 0; x < 120; ++x)
			if(x == 0)
				depth_data_x.at<float>(y,x) = 0.0;
			else if(normal.at<Vec3f>(y,x)[2] == 0.0)
				depth_data_x.at<float>(y,x) = 0.0;
			else
				depth_data_x.at<float>(y,x) = depth_data_x.at<float>(y,x-1) + partial_derive.at<Vec3f>(y,x)[0] *(-1);


	for(int x = 0; x < 120; x++)
		for ( int y = 0; y < 120; ++y)
			if(y == 0)
				depth_data_y.at<float>(y,x) = 0.0;
			else if(normal.at<Vec3f>(y,x)[2] == 0.0)
				depth_data_y.at<float>(y,x) = 0.0;
			else
				depth_data_y.at<float>(y,x) = depth_data_y.at<float>(y-1,x) + partial_derive.at<Vec3f>(y,x)[1] *(-1);
	...
	for(int x = 0; x < 120; x++)
		for ( int y = 0; y < 120; ++y)
			...
			{
				depth_data_sum.at<float>(x,y) = depth_data_sum.at<float>(x,y)*(-1);
			}

	//cout << depth_data_sum << endl;
...
*/
	...
	for (int i = 0; i < 120; ++i)
	{
		for (int j = 0; j < 120; ++j)
		{
			if(normal.at<Vec3f>(i,j)[2] == 0.0)
			{
				depth_data.at<float>(i,j) = 0.0;
			}
			else
			{
				depth_data.at<float>(i,j) = -(normal.at<Vec3f>(i,j)[0])*i / (normal.at<Vec3f>(i,j)[2]) - (normal.at<Vec3f>(i,j)[1])*j / (normal.at<Vec3f>(i,j)[2]);
				//depth_data.at<float>(i,j) = partial_derive.at<Vec3f>(i,j)[1]*(-i) + partial_derive.at<Vec3f>(i,j)[0]*(-j);
				if(depth_data.at<float>(i,j) < 0)
				{
					depth_data.at<float>(i,j) = depth_data.at<float>(i,j) / (10);
				}
				else
				{
					depth_data.at<float>(i,j) = depth_data.at<float>(i,j) / (-10);
