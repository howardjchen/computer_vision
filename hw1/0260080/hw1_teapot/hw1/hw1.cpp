...
{
	Mat afterflip(114,196,CV_32FC1,Scalar(0));
	if(type=='y')
	{
		for(int y=0;y<114;y++)
		{
			for(int x=0;x<196;x++)
			{
				afterflip.at<float>(113-y,x)=m.at<float>(y,x);
			}
		}
	}
	else if(type=='x')
	{
		for(int y=0;y<114;y++)
		{
			for(int x=0;x<196;x++)
			{
				afterflip.at<float>(y,195-x)=m.at<float>(y,x);
			}
		}
	}
	m=afterflip;
...
Mat partial_derive(114,196,CV_32FC3);
/////////////////////
///先從y方向積分/////
/////////////////////
double u=0;
...
void RemoveOffset(Mat &m)
{
	...
	x=A_inv*b;
	//cout<<x<<endl;
	Mat plane(114,196,CV_32FC1);
	for(int y=0;y<114;y++)
	{
		for(int xx=0;xx<196;xx++)
		{
...
int main( int argc, char** argv )
{
	...
	Mat b(114,196,CV_32FC3);
	Mat normal(114,196,CV_32FC3);
	Mat rho(114,196,CV_32FC1);
	...
	normal=normal*127;
	normal=normal+Mat(114,196,CV_32FC3,Scalar(127,127,127));
	Mat color_normal_map(114,196,CV_8UC3);
