...
void outputply( Mat &f)
{
	//FILE *fp1;
	fstream fp1;
	fp1.open("depth_map.ply",ios::out);
	...
	for (int i = 0; i < 212; ++i)
	{
		for (int j = 0; j < 120; ++j)
		{
			...
			fp1<< i <<" " << j  << " "<< f.at<float>(i,j) <<" " <<"255 255 255" << endl;
		}
	}
}
...
int main()
{
	...
	for (int i = 0; i < 212; ++i)
	{
		for (int j = 0; j < 120; ++j)
		{
			...
			{
				depth_data.at<float>(i,j) = -((normal.at<Vec3f>(i,j)[0]) / (normal.at<Vec3f>(i,j)[2])*i) - ((normal.at<Vec3f>(i,j)[1]) / (normal.at<Vec3f>(i,j)[2])*j);
				if(depth_data.at<float>(i,j) < 0)
				{
					depth_data.at<float>(i,j) = depth_data.at<float>(i,j) / (100);
				}
				else
				{
					depth_data.at<float>(i,j) = depth_data.at<float>(i,j) / (-100);
